...
* CFM - This is the memory for the Basic program. In the original
* code we search up from here and test whether we have valid RAM
* by reading the location, complementing the value, writing it
* back out and then comparing it to see if it's writable memory.
* since all our memory is RAM, that won't work and we'll
* overwrite the code. So make it a fixed size here

PROGSZ      EQU     20000
PROGST      RMB     PROGSZ              ; START OF PROGRAM SPACE
PROGEND     EQU     *-PROGST
...
* CFM - Clear memory down to 0. This used to clear the byte at $ffff
* which didn't matter because it was ROM. It's illegal memory for
* us so don't do that
*
BACDST    LDX  #PROGST+1      ; POINT X TO CLEAR 1ST 1K OF RAM
LA077     CLR  ,--X           ; MOVE POINTER DOWN AND CLEAR BYTE
          LEAX ,X             ; See if we are done
          BNE  LA077          ; KEEP GOING IF NOT AT BOTTOM OF PAGE 0

* CFM - Setup Basic program memory. See comment at PROGST.
* End of memory is PROGEND-1.
*
